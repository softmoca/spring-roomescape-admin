...
- 기존에는 요청과 응답 JSON 모양이 같아서 `Reservation` 클래스 하나로 *요청 받기*와 *응답 주기*를 둘 다 했었다. 하지만 3단계에서 둘이 갈라짐.
    - 요청: `timeId` (Long 하나)
    - 응답: `time: { id, startAt }` (객체)
- 요청과 응답 모양이 달라지면 DTO도입
- 도메인 객체 그대로 요청 응답에 사용하는건 안티패턴이다.
